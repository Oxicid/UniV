...
def face_select_set_func(sync: bool) -> typing.Callable[[BMFace, bool, BMLayerItem], typing.NoReturn]:
    if sync:
        def select_set(f, state, uv):  # noqa
            f.select = state
    else:
        def select_set(f, state, uv):
            for crn in f.loops:
                crn_uv = crn[uv]
                crn_uv.select = state
...
def face_select_linked_func(sync: bool, force=False, clamp_by_seams=False) -> typing.Callable[[BMFace, BMLayerItem], typing.NoReturn]:
    if force or clamp_by_seams:
        raise NotImplementedError()

    if sync:
        def select_set(f, uv):  # noqa
            f.select = True
    else:
        def select_set(f, uv):
            for crn in f.loops:
...
def edge_select_linked_set_func(sync: bool, force=False, clamp_by_seams=False) -> typing.Callable[[BMLoop, bool, BMLayerItem], typing.NoReturn]:
    # TODO: Add support clamp by seams
    if sync:
        def select_set(crn, state, uv):  # noqa
            crn.edge.select = state
    else:
        if force or clamp_by_seams:
            raise NotImplementedError()

        def select_set(crn: BMLoop, state, uv):
            crn_uv = crn[uv]
            if crn_uv.select_edge == state:
                return
...
def edge_select_linked_get_func(sync: bool) -> typing.Callable[[BMLoop, BMLayerItem], bool]:
    if sync:
        def select_get(crn, uv):  # noqa
            return crn.edge.select
    else:
        def select_get(crn, uv):
